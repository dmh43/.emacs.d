...
(defun activate-project-virtualenv ()
  (interactive)
  (pyvenv-activate (virtualenv-path))
  (flycheck-set-checker-executable (concat (virtualenv-path) "/bin/pylint"))
  (message "Activated %s" (virtualenv-path)))

(defun nose-virtualenv-test-all ()
  (interactive)
  (let ((nose-global-name (concat (virtualenv-path) "/bin/nosetests")))
    (nosetests-all)))
